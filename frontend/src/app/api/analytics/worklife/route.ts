  ...
  try {
    ...
    focusSessions.forEach(s => {
      ...
      if (focusByDay[day] !== undefined) {
        focusByDay[day] += Math.round(s.duration / 60);
      }
    });
    ...
    // 6. Task distribution by project
    ...
    // 7. Work-Life Balance Score (0-100)
    const today = new Date().toISOString().split('T')[0];
    const avgFocusMinutes = Object.values(focusByDay).reduce((a, b) => a + b, 0) / days;
    const focusScore = Math.min(25, (avgFocusMinutes / 120) * 25);
